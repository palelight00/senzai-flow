...
- **分析タブ**。平均洗剤使用量（全体/年/季節/月）、各種グラフ、秋田市の気温との相関を表示（外部ライブラリ不使用の自作SVGグラフ）。
...
外気温との相関を見るため、秋田市の日別 最高/最低/平均気温を取り込めます。

- **自動取得**: 「秋田市の気温を自動取得」ボタンで [Open-Meteo](https://open-meteo.com/) の無料・鍵不要の過去気象APIから取得します（ブラウザから直接通信。アーカイブの仕様上、直近約5日は未掲載）。
...
```jsonc
{
  ...
  "weather": {
    "schemaVersion": 1,
    "location": { "name": "秋田市", "lat": 39.72, "lon": 140.1 },
